...
module.controller('Main', ['$scope', 'Core', 'hs.query.baseService', 'hs.compositions.service_parser', '$timeout', 'hs.map.service', '$http', 'config', '$rootScope', 'hs.utils.service', '$compile', 'hs.query.baseService', '$sce',
    function ($scope, Core, QueryService, composition_parser, $timeout, hsMap, $http, config, $rootScope, utils, $compile, query_service, $sce) {
        ...
        function getOlus() {
            ...
            var q = 'https://www.foodie-cloud.org/sparql?default-graph-uri=&query=' + encodeURIComponent('PREFIX geo: <http://www.opengis.net/ont/geosparql#> PREFIX geof: <http://www.opengis.net/def/function/geosparql/> PREFIX virtrdf: <http://www.openlinksw.com/schemas/virtrdf#> PREFIX poi: <http://www.openvoc.eu/poi#> PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#> SELECT ?o ?use ?wkt (COUNT(*) as ?poi_count) FROM <http://www.sdi4apps.eu/poi.rdf> WHERE {?Resource geo:asWKT ?Coordinates . FILTER(bif:st_may_intersect (?Coordinates, ?wkt)). { SELECT ?o ?wkt ?use FROM <http://w3id.org/foodie/olu#> WHERE { ?o geo:hasGeometry ?geometry. ?geometry geo:asWKT ?wkt. FILTER(bif:st_may_intersect(bif:st_geomfromtext("BOX(' + extents + ')"), ?wkt)). ?o <http://w3id.org/foodie/olu#specificLandUse> ?use. } } }') + '&should-sponge=&format=application%2Fsparql-results%2Bjson&timeout=0&debug=on';
                    ...
                    for (var i = 0; i < response.results.bindings.length; i++) {
                        try {
                            ...
                            if (b.wkt.datatype == "http://www.openlinksw.com/schemas/virtrdf#Geometry" && b.wkt.value.indexOf('e+') == -1 && b.wkt.value.indexOf('e-') == -1) {
                                ...
                                var feature = new Feature({ geometry: geom_transformed, parcel: b.o.value, use: b.use.value, poi_count: b.poi_count.value });
                    ...
                    for (var i = 0; i < response.results.bindings.length; i++) {
                        try {
                            ...
                            if (b.coordinatesOLU.datatype == "http://www.openlinksw.com/schemas/virtrdf#Geometry" && b.coordinatesOLU.value.indexOf('e+') == -1 && b.coordinatesOLU.value.indexOf('e-') == -1) {
                                ...
                                var ext = g_feature.getGeometry().getExtent()
                                var geom_transformed = g_feature.getGeometry().transform('EPSG:4326', hsMap.map.getView().getProjection());
                                var feature = new Feature({ geometry: geom_transformed, parcel: b.olu.value, use: b.specificLandUse.value, poi_count: 30 });
                                features.push(feature);
                            }
                        } catch (ex) {
                            console.log(ex);
        ...
        function getLinksTo(id, callback) {
            var q = 'https://www.foodie-cloud.org/sparql?default-graph-uri=&query=' + encodeURIComponent('PREFIX geo: <http://www.opengis.net/ont/geosparql#> PREFIX geof: <http://www.opengis.net/def/function/geosparql/> PREFIX virtrdf: <http://www.openlinksw.com/schemas/virtrdf#> PREFIX poi: <http://www.openvoc.eu/poi#> PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#> SELECT * FROM <http://www.sdi4apps.eu/poi.rdf> WHERE {?poi geo:asWKT ?Coordinates . FILTER(bif:st_may_intersect (?Coordinates, ?wkt)). { SELECT ?wkt FROM <http://w3id.org/foodie/olu#> WHERE { <' + id + '> geo:hasGeometry ?geometry. ?geometry geo:asWKT ?wkt.} } }') + '&should-sponge=&format=application%2Fsparql-results%2Bjson&timeout=0&debug=on';
            $.ajax({
                url: q
            })
                .done(function (response) {
